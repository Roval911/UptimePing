...
- **Статус:** ✅ **ПРОВЕРЕНО (2026-03-30)** - Возвращает `201 Created` и реальные `tenant_id/user_id` (создаются в Postgres), refresh-сессия сохраняется в Redis. Все исправления notification-service успешно применены. 
...
- **Статус:** ✅ **ПРОВЕРЕНО (2026-03-30)** - Возвращает `200 OK`, работает с пользователем из Postgres, refresh-сессия в Redis
...
- **Статус:** ✅ **ПРОВЕРЕНО (2026-03-30)** - Возвращает `200 OK`, refresh реально валидируется по сессии в Redis и ротируется
...
- **Статус:** ✅ **ПРОВЕРЕНО (2026-03-30)** - `GET` возвращает `405` (как и заявлено). `POST /api/v1/auth/api-keys` реализован без моков: создаёт API key в Postgres и возвращает `201` с реальными `key/secret`.
...
- **Статусы:** `200` (успех), `401` (неавторизован), `500` (ошибка)
- **Статус:** ✅ **ПРОВЕРЕНО (2026-03-30)** - Возвращает `200 OK` с реальными данными (пустой список при отсутствии)
...
- **Статус:** ✅ **ПРОВЕРЕНО (2026-03-30)** - Возвращает `201 Created` когда используется правильный формат: `POST /api/v1/schedules/{check_id}` с `{"cron_expression":"string"}`. Расписание сохраняется в Postgres с реальными данными.
...
- **Статус:** ✅ **ПРОВЕРЕНО (2026-03-30)** - Возвращает `200 OK` с реальными данными из Postgres, работает с существующими schedule ID
...
- **Статус:** ✅ **ПРОВЕРЕНО (2026-03-30)** - Возвращает `200 OK` и корректно обновляет cron_expression в Postgres. Расписание успешно обновляется.
...
- **Статусы:** `200` (успех), `404` (не найден), `401` (неавторизован), `500` (ошибка)
- **Статус:** ✅ **ПРОВЕРЕНО (2026-03-30)** - Возвращает `200 OK` с сообщением `{"success": true, "message": "Check unscheduled"}`. Расписание успешно удаляется из Postgres.
...
- **Статус:** ✅ **ПРОВЕРЕНО (2026-03-30)** - Возвращает `200 OK` с реальными данными выполнения проверки. Core Service выполняет HTTP проверку, сохраняет результат в Postgres и кеширует в Redis.
...
- **Статусы:** `200` (успех), `400` (невалидный ID), `401` (неавторизован), `500` (ошибка)
- **Статус:** ✅ **ПРОВЕРЕНО (2026-03-30)** - Возвращает `200 OK` со статусом проверки из кеша или базы данных
...
- **Статусы:** `200` (успех), `400` (невалидный ID), `401` (неавторизован), `500` (ошибка)
- **Статус:** ✅ **ПРОВЕРЕНО (2026-03-30)** - Возвращает `200 OK` с реальной историей выполнений из Postgres
...
- **Статус:** ✅ **ПРОВЕРЕНО (2026-03-30)** - Возвращает `200 OK` с корректной конфигурацией. Проблема с nil значениями в check_config полностью исправлена, значения по умолчанию работают корректно.
...
- **Статусы:** `200` (успех), `400` (валидация), `401` (неавторизован), `500` (ошибка)
- **Статус:** ✅ **ПРОВЕРЕНО (2026-03-30)** - Возвращает `200 OK` с корректным парсингом proto файла.
...
- **Статусы:** `200` (успех), `401` (неавторизован), `500` (ошибка)
- **Статус:** ✅ **ПРОВЕРЕНО (2026-03-30)** - Возвращает `200 OK` с реальными метриками со всех сервисов.
---
...
- **Статусы:** `200` (успех), `400` (валидация), `401` (неавторизован), `500` (ошибка)
- **Статус:** ✅ **ПРОВЕРЕНО (2026-03-30)** - Возвращает `200 OK` с корректным JSON ответом и временной меткой.
...
- **Статус:** ✅ **ПРОВЕРЕНО (2026-03-30)** - Возвращает `200 OK` с пустым списком инцидентов (корректно при их отсутствии).
...
- **Статусы:** `201` (создан), `400` (валидация), `401` (неавторизован), `500` (ошибка)
- **Статус:** ✅ **ПРОВЕРЕНО (2026-03-30)** - Возвращает `201 Created` с реальными данными инцидента, сохраняет в PostgreSQL
...
- **Статус:** ✅ **ПРОВЕРЕНО (2026-03-30)** - Возвращает `200 OK` с реальными данными инцидента из PostgreSQL
...
- **Статус:** ✅ **ПРОВЕРЕНО (2026-03-30)** - Возвращает `200 OK` и обновляет инцидент в PostgreSQL
...
- **Запрос:** `{"message": "string", "channel_id": "string", "priority": "string", "recipient": "string"}`
- **Ответ:** `{"success": boolean, "message_id": "string", "timestamp": "string"}`
- **Статусы:** `200` (успех), `400` (валидация), `401` (неавторизован), `500` (ошибка)
- **Статус:** ✅ **ПРОВЕРЕНО (2026-03-30)** - Возвращает `200 OK` с успешной отправкой уведомления.
---
...
- **Статусы:** `200` (успех), `401` (неавторизован), `500` (ошибка)
- **Статус:** ✅ **ПРОВЕРЕНО (2026-03-30)** - Возвращает `200 OK` с реальными данными каналов из PostgreSQL.
...
- **Статусы:** `201` (создан), `400` (валидация), `401` (неавторизован), `500` (ошибка)
- **Статус:** ✅ **ПРОВЕРЕНО (2026-03-30)** - Возвращает `200 OK` с успешным созданием канала в PostgreSQL.
...
- **Статус:** ✅ **ПРОВЕРЕНО (2026-03-30)** - Возвращает `200 OK` с корректным JSON и данными сервиса.
...
- **Статус:** ✅ **ПРОВЕРЕНО (2026-03-30)** - Возвращает `200 OK` со статусом "ready" и timestamp.
...
- **Статус:** ✅ **ПРОВЕРЕНО (2026-03-30)** - Возвращает `200 OK` со статусом "alive" и timestamp.
...
- **Статус:** ✅ **ПРОВЕРЕНО (2026-03-30)** - Проксируется через API Gateway, возвращает `200 OK`.
