...
- **Статус:** ✅ **ПРОВЕРЕНО (2026-03-18)** - Возвращает `201 Created` с реальными данными проверки, сохраняет в Postgres с tenant_id. config поле не требуется (базовая конфигурация по умолчанию).
...
- **Статус:** ✅ **ПРОВЕРЕНО (2026-03-18)** - Возвращает `200 OK` с реальными данными из Postgres, фильтрует по tenant_id пользователя
...
- **Статус:** ✅ **ПРОВЕРЕНО (2026-03-18)** - Возвращает `200 OK` с реальными данными из Postgres, проверяет tenant_id доступа
...
- **Статус:** ✅ **ПРОВЕРЕНО (2026-03-18)** - Возвращает `200 OK` с обновленными данными проверки
...
- **Статусы:** `200` (успех), `400` (невалидный ID), `401` (неавторизован), `403` (доступ запрещен), `404` (не найден), `500` (ошибка)
- **Статус:** ✅ **ПРОВЕРЕНО (2026-03-18)** - Возвращает `200 OK` и реально удаляет проверку из Postgres
...
- **Статусы:** `200` (успех), `401` (неавторизован), `500` (ошибка)
- **Статус:** ✅ **ПРОВЕРЕНО (2026-03-18)** - Возвращает `200 OK` с реальными данными (пустой список при отсутствии)
...
- **Статус:** ✅ **ПРОВЕРЕНО (2026-03-18)** - Возвращает `201 Created` когда используется правильный формат: `POST /api/v1/schedules/{check_id}` с `{"cron_expression":"string"}`. Расписание сохраняется в Postgres с реальными данными.
- **Статус:** ✅ **ПРОВЕРЕНО (2026-03-18)** - Возвращает `200 OK` с реальными данными из Postgres, работает с существующими schedule ID
...
- **Статус:** ✅ **ПРОВЕРЕНО (2026-03-18)** - Возвращает `200 OK` и корректно обновляет cron_expression в Postgres. Расписание успешно обновляется.
...
- **Статусы:** `200` (успех), `404` (не найден), `401` (неавторизован), `500` (ошибка)
- **Статус:** ✅ **ПРОВЕРЕНО (2026-03-18)** - Возвращает `200 OK` с сообщением `{"success": true, "message": "Check unscheduled"}`. Расписание успешно удаляется из Postgres.
...
- **Статус:** ✅ **ПРОВЕРЕНО (2026-03-18)** - Возвращает `200 OK` с реальными данными выполнения проверки. Core Service выполняет HTTP проверку, сохраняет результат в Postgres и кеширует в Redis.
...
- **Статусы:** `200` (успех), `400` (невалидный ID), `401` (неавторизован), `500` (ошибка)
- **Статус:** ✅ **ПРОВЕРЕНО (2026-03-18)** - Возвращает `200 OK` со статусом проверки из кеша или базы данных
...
- **Статусы:** `200` (успех), `400` (невалидный ID), `401` (неавторизован), `500` (ошибка)
- **Статус:** ✅ **ПРОВЕРЕНО (2026-03-18)** - Возвращает `200 OK` с реальной историей выполнений из Postgres
...
4. **🏥 Health Checks (4/7)** - базовые health checks работают

### ⚠️ **ЧАСТИЧНО РАБОЧИЕ КАТЕГОРИИ:**
5. **⚙️ Config Service (0/1)** - 403 ошибка авторизации
6. **🔧 Forge Service (0/4)** - 500 ошибка сервера

### ✅ **ПОЛНОСТЬЮ ПРОВЕРЕННЫЕ КАТЕГОРИИ:**
7. **⚡ Core Service (3/3)** - все операции работают, результаты сохраняются в БД
