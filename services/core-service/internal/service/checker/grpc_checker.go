...
func (g *gRPCChecker) Execute(task *domain.Task) (*domain.CheckResult, error) {
	...
	statusCode := &[]int{200}[0] // gRPC не имеет HTTP статус кодов, используем 200 для успеха
	...
	result := &domain.CheckResult{
		...
		ResponseBody:    "", // gRPC response body
		ResponseHeaders: make(map[string]string),
		CreatedAt:       time.Now().UTC(),
	}

	// Добавление метаданных в заголовки
	result.ResponseHeaders["address"] = address
	result.ResponseHeaders["service"] = grpcConfig.Service
	result.ResponseHeaders["method"] = grpcConfig.Method

	if !success {
		result.ErrorMessage = "gRPC health check failed"
